...
describe("Codex hook installation", () => {
  it("preserves existing handlers and stays idempotent", () => {
    ...
    const once = mergedCodexHooks(existing, "/usr/bin/node", "/opt/jev/dist/cli.js", "jev-use-typesafe");
    const twice = mergedCodexHooks(once, "/usr/bin/node", "/opt/jev/dist/cli.js", "jev-use-typesafe");
    ...
    expect(twice.hooks!.PreToolUse[0].hooks![0].command).toContain("JEV_TYPESAFE_KEYCHAIN_SERVICE='jev-use-typesafe'");
